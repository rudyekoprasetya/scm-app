                    ...
                    <form action="{{ route('users.update', $user) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input name="name" value="{{ old('name', $user->name) }}" class="w-full mt-1" required />
                                <x-input-error :messages="$errors->get('name')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Email" />
                                <x-text-input name="email" type="email" value="{{ old('email', $user->email) }}" class="w-full mt-1" required />
                                <x-input-error :messages="$errors->get('email')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Password (kosongkan jika tidak diubah)" />
                                <x-text-input name="password" type="password" class="w-full mt-1" />
