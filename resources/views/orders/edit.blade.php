<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Pesanan') }}</h2>
    </x-slot>
                    ...
                    <form action="{{ route('orders.update', $order) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <x-text-input name="customer_name" value="{{ old('customer_name', $order->customer_name) }}" class="w-full mt-1" required />
                                <x-input-error :messages="$errors->get('customer_name')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Email Pelanggan" />
                                <x-text-input name="customer_email" type="email" value="{{ old('customer_email', $order->customer_email) }}" class="w-full mt-1" />
                                <x-input-error :messages="$errors->get('customer_email')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Telepon Pelanggan" />
                                <x-text-input name="customer_phone" value="{{ old('customer_phone', $order->customer_phone) }}" class="w-full mt-1" />
                                <x-input-error :messages="$errors->get('customer_phone')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Tanggal Order" />
                                <x-text-input name="order_date" type="date" value="{{ old('order_date', $order->order_date?->format('Y-m-d')) }}" class="w-full mt-1" required />
                                <x-input-error :messages="$errors->get('order_date')" class="mt-1" />
                            </div>
                            <div class="md:col-span-2">
                                <x-input-label value="Alamat Pengiriman" />
                                <textarea name="shipping_address" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1" required>{{ old('shipping_address', $order->shipping_address) }}</textarea>
                                <x-input-error :messages="$errors->get('shipping_address')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Pajak" />
                                <x-text-input name="tax" type="number" step="0.01" value="{{ old('tax', $order->tax) }}" class="w-full mt-1" />
                                <x-input-error :messages="$errors->get('tax')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Biaya Kirim" />
                                <x-text-input name="shipping_cost" type="number" step="0.01" value="{{ old('shipping_cost', $order->shipping_cost) }}" class="w-full mt-1" />
                                <x-input-error :messages="$errors->get('shipping_cost')" class="mt-1" />
                            </div>
                            <div class="md:col-span-2">
                                <x-input-label value="Catatan" />
                                <textarea name="notes" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1">{{ old('notes', $order->notes) }}</textarea>
                                    ...
                                    <tr class="item-row">
                                        <td class="px-4 py-2">
                                            <select name="items[{{ $i }}][product_id]" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full text-sm" required>
                                                ...
                                                <option value="{{ $p->id }}" {{ $item->product_id == $p->id ? 'selected' : '' }} data-price="{{ $p->selling_price }}">{{ $p->name }} ({{ $p->sku }})</option>
                                                @endforeach
                                            </select>
                                        </td>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('orders.show', $order) }}"><x-secondary-button type="button">Batal</x-secondary-button></a>
                        </div>
                    </form>
                </div>
            </div>
