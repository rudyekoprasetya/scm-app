                    ...
                    <form action="{{ route('products.update', $product) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input name="name" value="{{ old('name', $product->name) }}" class="w-full mt-1" required />
                                <x-input-error :messages="$errors->get('name')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="SKU" />
                                <x-text-input name="sku" value="{{ old('sku', $product->sku) }}" class="w-full mt-1" required />
                                <x-input-error :messages="$errors->get('sku')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Satuan" />
                                <x-text-input name="unit" value="{{ old('unit', $product->unit) }}" class="w-full mt-1" />
                                <x-input-error :messages="$errors->get('unit')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Harga Beli" />
                                <x-text-input name="purchase_price" type="number" step="0.01" value="{{ old('purchase_price', $product->purchase_price) }}" class="w-full mt-1" />
                                <x-input-error :messages="$errors->get('purchase_price')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Harga Jual" />
                                <x-text-input name="selling_price" type="number" step="0.01" value="{{ old('selling_price', $product->selling_price) }}" class="w-full mt-1" />
                                <x-input-error :messages="$errors->get('selling_price')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Stok" />
                                <x-text-input name="stock_quantity" type="number" value="{{ old('stock_quantity', $product->stock_quantity) }}" class="w-full mt-1" />
                                <x-input-error :messages="$errors->get('stock_quantity')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label value="Batas Stok Minimum" />
                                <x-text-input name="low_stock_threshold" type="number" value="{{ old('low_stock_threshold', $product->low_stock_threshold) }}" class="w-full mt-1" />
                                <x-input-error :messages="$errors->get('low_stock_threshold')" class="mt-1" />
                            </div>
                            <div class="md:col-span-2">
                                <x-input-label value="Deskripsi" />
                                <textarea name="description" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1">{{ old('description', $product->description) }}</textarea>
